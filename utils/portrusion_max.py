...
def max_protrusion_array(protrs: list[list[float]]) -> list[list[float]]:
    """
    ...
    max_value = protrs_np[max_index, 1]
    protrs_np[:, 1] = 0.0
    protrs_np[max_index, 1] = max_value
    return protrs_np.tolist()

def max_protrusion_amount(protrs: list[list[float]]) -> float:
    """
    Get the protrusion amount with the maximum size.
    :param protrs: Protrusion array with size MAX_ACTIVE_PROTRUSIONS and elements [amount, size].
    :return: The amount of protrusions with the maximum size.
    """
    if not protrs:
        return 0.0
    protrs_np = np.array(protrs, dtype=np.float32)
    max_index = np.argmax(protrs_np[:, 1])
    return protrs_np[max_index, 0] if max_index < len(protrs_np) else 0.0